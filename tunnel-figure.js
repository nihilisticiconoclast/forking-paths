/* ─────────────────────────────────────────────────────────────────────────
   ...
   Returns a deterministic SVG *string* from a seed (typically a page slug or
   title). No dependencies; runs unchanged in the browser and in Node.

       same seed  -> identical figure   (static per page)
       new  seed  -> different terrain  (of a theme, not a clone)

   It is a REAL contour plot: a seeded scalar field (a barrier ridge between
   two basins) is contoured with marching squares, then a route is drawn
   through the ridge using the Ordnance-Survey tunnel convention (solid →
   dashed-through-the-wall → solid, with portal ticks), and a small WKB inset
   shows the amplitude surviving the barrier. Amber appears only on the high
   contours — the one element borrowed from the phase-space palette.

   Usage (browser):
       el.innerHTML = TunnelFigure.tunnelFigureSVG("statistical-rethinking");

   Usage (Node):
       const { tunnelFigureSVG } = require("./tunnel-figure.js");
   ───────────────────────────────────────────────────────────────────────── */
(function (root, factory) {
  ...
  function tunnelFigureSVG(seed, opts) {
    ...
    const ridgeX     = rand(0.42, 0.58);   // barrier centre (x, normalised)
    const ridgeWidth = rand(0.065, 0.115); // barrier thickness
    const ridgeBend  = rand(-0.18, 0.18);  // curvature of the ridge
    const ridgeTilt  = rand(-0.45, 0.45);  // lean
    const ridgeH     = rand(1.00, 1.35);   // crest height
    const bx1 = rand(0.13, 0.30), by1 = rand(0.34, 0.66); // basin 1 (left)
    const bx2 = rand(0.70, 0.87), by2 = rand(0.34, 0.66); // basin 2 (right)
    ...
    function fieldAt(u, v) {
      ...
      f += 0.12 * (v - 0.5);  // gentle global tilt
      return f;
    }
    ...
    function march(level) {
      ...
      for (let j = 0; j < NY - 1; j++) for (let i = 0; i < NX - 1; i++) {
        ...
        switch (idx) {
          ...
          case 5:  segs.push([L, T]); segs.push([B, R]); break; // saddle
    ...
    for (let k = 1; k < NLEV; k++) {
      ...
      if (t > 0.72)        { stroke = PAL.amber; w = 1.4; op = 0.9;  } // high ground = energy/amplitude
      else if (k === indexK) { stroke = PAL.index; w = 1.7; op = 0.85; } // an index contour
      contourSVG += '<path d="' + d + '" fill="none" stroke="' + stroke
                 + '" stroke-width="' + w + '" opacity="' + op + '"/>';
    }

    // ── the route: basin 1 → through the ridge crest → basin 2 ──────────────
    const crestLevel = fmin + 0.72 * (fmax - fmin);  // matches the amber band
    const steps = 60;
    const route = [];
    ...
    // ── WKB inset: barrier width sets the decay; wavelength is seeded ───────
    ...
    // ── seeded labels: a grid reference and a spot height ───────────────────
    ...
    const defs =
        '<defs><marker id="tnl-ar" markerWidth="9" markerHeight="9" refX="6" refY="4.5" orient="auto">'
      + '<path d="M0 0 L9 4.5 L0 9 Z" fill="' + PAL.route + '"/></marker></defs>';
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" fill="none" xmlns="http://www.w3.org/2000/svg" '
      ...
      + defs + contourSVG + routeSVG + insetSVG + labelSVG
      + "</svg>";
  }
